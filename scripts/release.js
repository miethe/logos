...
const prettyPrintCommit = async (commit) => {
    ...
    // to
    //
    //   ✨ Publish `Logo Name` logo on `npm`

    if (commit.tag === '🚀') {
        if (commit.title.indexOf('v1.0.0') !== -1 ) {
            const logoName = commit.title.split(' ')[1];

            commitTitle = `✨ Publish \`${prettyPrintLogoName(logoName)}\` logo on \`npm\``;
        } else {
            return '';
        }
    }
